...
if(!function_exists('etheme_init')) {
    function etheme_init() {
        global $etheme_responsive;
        $etheme_get_chosen_google_font = etheme_get_chosen_google_font();
...
if (!function_exists('etheme_logo')) {
    function etheme_logo( $logo = 'logo' ) {
...
if( ! function_exists( 'et_register_action' ) ) {
	function et_register_action() {
		...
		if( ! $captcha_instance->check( $captcha_prefix, $captcha_word ) ) {
			$return['status'] = 'error';
			$return['msg'] = esc_html__( 'The security code you entered did not match. Please try again.', ETHEME_DOMAIN );
			echo json_encode($return);
			die();
		}

	    if( ! empty( $et_register ) ){

	        //We shall SQL escape all inputs
	        $username = esc_sql( $username );
	        if( empty( $username ) ) {
				$return['status'] = 'error';
				$return['msg'] = esc_html__( 'User name should not be empty.', ETHEME_DOMAIN );
				echo json_encode($return);
	            die();
	        }
	        ...
	        if( ! preg_match( "/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$/", $email ) ) {
				$return['status'] = 'error';
				$return['msg'] = esc_html__( 'Please enter a valid email.', ETHEME_DOMAIN );
				echo json_encode($return);
	            die();
	        }
	        ...
	        if( empty( $pass ) || strlen( $pass ) < 5) {
				$return['status'] = 'error';
				$return['msg'] = esc_html__( 'Password should have more than 5 symbols', ETHEME_DOMAIN );
				echo json_encode($return);
	            die();
	        }
	        if( $pass != $pass2 ) {
				$return['status'] = 'error';
				$return['msg'] = esc_html__( 'The passwords do not match', ETHEME_DOMAIN );
				echo json_encode($return);
	            die();
	        }
			
			$etterms = esc_sql( $terms );
	        if( empty( $etterms ) ) {
				$return['status'] = 'error';
				$return['msg'] = esc_html__( '이용약관에 동의하셔야 합니다.', ETHEME_DOMAIN );
				echo json_encode($return);
	            die();
	        }
	        ...
	        if ( is_wp_error( $status ) ) {
				$return['status'] = 'error';
				$return['msg'] = esc_html__( 'Username already exists. Please try another one.', ETHEME_DOMAIN );
				echo json_encode($return);
	        } else {
				...
				$return['msg'] = esc_html__( 'Please check your email for login details.', ETHEME_DOMAIN );
				echo json_encode($return);
	        }
	        die();
	    }
					...
					<?php
						while ( $query->have_posts() ) {
							$query->the_post();
                             ...
                             if ( etheme_get_option( 'search_products' ) && etheme_get_option( 'search_out_products' ) && get_post_type() == 'product' ) {
			    ...
			    $link_image_end .= '
						...
						<div class="block-mask">
							<div class="mask-content">
								<a href="'.get_the_post_thumbnail_url($post_id).'" rel="lightbox"><i class="icon-resize-full"></i></a>
...
if(!function_exists('etheme_create_flex_slider')) {
    function etheme_create_flex_slider($args,$title = false, $shop_link = true, $sidebar_slider = false){
        ...
        if(count($multislides) > 1) {
            $class .= ' posts-count-gt1';
        }
        if(count($multislides) < 5) {
            $class .= ' posts-count-lt5';
        }
       if ( count($multislides) ) :
            if ($title) {
                                ...
                                foreach ( $args as $upsell ) :
                                    ...
                                    if(class_exists('Woocommerce')) {
                                        ...
                                        if (!$product->is_visible()) continue;
                                            ...
                                            wc_get_template_part( 'content', 'product' );
                                        if($sidebar_slider){
                                            if($_i%2 == 0 && $_i != count($multislides) ) {
                                                echo '</div><!-- slide-item -->';
                                                echo '<div class="slide-item product-slide">';
                                            }
                                        } else {
...
<div class="page-heading bc-type-<?php etheme_option('breadcrumb_type'); ?> " style="<?php echo $style; ?> ">
        <div class="container" <?php echo $bk_color; ?>>
                ...
                <div class="span12 a-center">
                    ...
                    <?php if (et_is_woo_exists()&&(is_woocommerce()||is_cart()||is_checkout()||is_product_category()||is_product_tag())): ?>
                        ...
                        <?php if ( apply_filters( 'woocommerce_show_page_title', true ) ) : ?>
                            ...
                            <?php else: ?>
                                 <h1 class="title"><span <?php echo $bk_color; ?>><?php the_title(); ?></span></h1>
                            <?php endif; ?>
                            ...
                            <span <?php echo $bk_color; ?>>
                                <?php 
                                    if( is_home() && get_option( 'page_for_posts' ) ) {
                                        ...
                                        esc_html_e( 'Not found', ETHEME_DOMAIN );
                                    } else {
                                        the_title();
                                    }
                                 ?>
