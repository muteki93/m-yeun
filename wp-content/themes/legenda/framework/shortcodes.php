	...
	function et_brands($atts) {
		...
		if ($count > 0) {
		    ...
		    foreach ($terms as $term) {
				...
				if($thumbnail_id) {
					$output .= '<a href="' . get_term_link( $term ) . '" title="' . sprintf(__('View all products from %s', ETHEME_DOMAIN), $term->name) . '">' . new_etheme_get_image( $thumbnail_id, 'medium' ) . '</a>';
				} else {
...
function etheme_countdown_shortcode($atts) {
    ...
    return '<div class="et-timer'.$class.'" data-final="'.$date.'">
                <div class="time-block">
                    ...
                    ' . esc_html__( 'days', ETHEME_DOMAIN ) . '
                </div>
                <div class="timer-devider">&nbsp;</div>
                <div class="time-block">
                    <span class="hours"></span>
                    ' . esc_html__( 'hours', ETHEME_DOMAIN ) . '
                </div>
                <div class="timer-devider">&nbsp;</div>
                <div class="time-block">
                    <span class="minutes"></span>
                    ' . esc_html__( 'minutes', ETHEME_DOMAIN ) . '
                </div>
                <div class="timer-devider">&nbsp;</div>
                <div class="time-block">
                    <span class="seconds"></span>
                    ' . esc_html__( 'seconds', ETHEME_DOMAIN ) . '
                </div>
                <div class="clear"></div>
            </div>';
}
...
if ( ! function_exists( 'etheme_progress_shortcode' ) ) :
function etheme_progress_shortcode($atts) {
    ...
    return '<div class="et-progress-bars"><div class="progress-bar" data-width="'.$a['complete'].'" style="'.$style.'"><span>'.$a['title'].'</span><div></div></div></div>';
}
endif;
...
if ( ! function_exists( 'etheme_add_shortcodes_buttons' ) ) :
...
if ( ! function_exists( 'etheme_add_shortcodes_buttons2' ) ) :
