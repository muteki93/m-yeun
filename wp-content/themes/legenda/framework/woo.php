			        ...
			        foreach($terms as $brand) {
			        	...
			        	if ($thumbnail_id) : ?>
		                	<a href="<?php echo get_term_link($brand); ?>">
	                    		<img src="<?php echo wp_get_attachment_image_url( $thumbnail_id, 'full' ); ?>" title="<?php echo $brand->name; ?>" alt="<?php echo $brand->name; ?>" class="brand-image" />
		                	</a>
			            <?php else : ?>
				...
				<div>
					...
					<div class="modal-body">
						...
						<form method="post" class="login">
                            ...
                            <?php else: ?>
                                <a href="http://m-yeun.co.kr/wordpress/register/" class="right"><?php _e('Create Account', ETHEME_DOMAIN) ?></a>
                            <?php endif ?>
							</p>
						</form>
					</div>
...
if(!function_exists('et_cart_summ')) {
	function et_cart_summ() {
        global $woocommerce;
        ?>
			<a href="http://m-yeun.co.kr/cart" class="cart-summ" data-items-count="<?php echo WC()->cart->get_cart_contents_count(); ?>">
				<div class="cart-bag">
...
if(!function_exists('etheme_cart_items')) {
	function etheme_cart_items ($limit = 3) {
					...
					<li class="woocommerce-mini-cart-item <?php echo esc_attr( apply_filters( 'woocommerce_mini_cart_item_class', 'mini_cart_item', $cart_item, $cart_item_key ) ); ?> product-item-popup">
						<?php 
                            echo apply_filters( 'woocommerce_cart_item_remove_link', 
                            	...
                            	sprintf('<a href="%s" aria-label="%s" data-product_id="%s" data-product_sku="%s" data-key="%s" class="remove close-order-li delete-btn" title="%s"><i class="icon-remove"></i></a>', 
                            		esc_url( $woocommerce->cart->get_remove_url( $cart_item_key ) ),
                            		esc_html__( 'Remove this item', ETHEME_DOMAIN ) ,
                            		esc_attr( $product_id ),
                            		esc_attr( $_product->get_sku() ),  
                            		$cart_item_key, 
                            		esc_html__( 'Remove this item', ETHEME_DOMAIN ) ), $cart_item_key );
						...
						<div class="media">
							...
							<div class="media-body">
								...
								<div class="descr-box">
									...
									<span class="coast"><?php esc_html_e( 'Qty: ', ETHEME_DOMAIN ); echo $cart_item['quantity']; ?><span class='medium-coast pull-right'><?php echo $product_price; ?></span></span>
								</div>
							</div>
						</div>
					</li>
        ...
        <?php   
            ...
            echo '<p class="woocommerce-mini-cart__empty-message empty a-center">' . esc_html__( 'No products in the cart.', ETHEME_DOMAIN ) . '</p>';
        }
...
if(!function_exists('et_step_checkout')) {
	function et_step_checkout(){
		$messages = array( 
			'password'    => __( 'CREATE AN ACCOUNT PASSWORD IS A REQUIRED FIELD.', ETHEME_DOMAIN ),
			'first_name'  => __( 'BILLING FIRST NAME IS A REQUIRED FIELD.', ETHEME_DOMAIN ), 
			'last_name'	  => __( 'BILLING LAST NAME IS A REQUIRED FIELD.', ETHEME_DOMAIN ), 
			'email_field' => __( 'BILLING EMAIL ADDRESS IS A REQUIRED FIELD.', ETHEME_DOMAIN ), 
			'phone' 	  => __( 'BILLING PHONE REQUIRED FIELD.', ETHEME_DOMAIN ), 
			'country' 	  => __( 'BILLING STATE IS A REQUIRED FIELD.', ETHEME_DOMAIN ), 
			'address_1'   => __( 'BILLING ADDRESSIS A REQUIRED FIELD.', ETHEME_DOMAIN ), 
			'city' 		  => __( 'BILLING TOWN / CITY IS A REQUIRED FIELD.', ETHEME_DOMAIN ), 
			'postcode'    => __( 'BILLING POSTCODE / ZIP A REQUIRED FIELD.', ETHEME_DOMAIN ), 
		);
